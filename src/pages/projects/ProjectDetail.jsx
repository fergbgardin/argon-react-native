...
export default function ProjectDetail() {
  ...
  // Financial summary
  const totalRecebido = concludedSessions.reduce((sum, s) => {
    return sum + (s.session_payments || []).reduce((a, p) => a + (p.valor || 0), 0)
  }, 0)

  const saldoDevedor = project.valor_total
    ? Math.max(project.valor_total - totalRecebido, 0)
    : null
        ...
        <Card className="p-4 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-muted">
              {project.tipo_cobranca === 'fechado' ? 'Projeto fechado' : 'Cobrança por sessão'}
            </span>
          </div>
              ...
              {project.valor_total && (
                <div className="flex justify-between text-sm">
                  <span className="text-muted">
                    {project.tipo_cobranca === 'fechado' ? 'Valor combinado' : 'Estimativa'}
                  </span>
                    ...
                    <div className="text-right">
                      ...
                      {s.status === 'concluida' && (
                        <p className="text-xs text-white mt-1">
                          {formatCurrency(
                            (s.session_payments || []).reduce((sum, p) => sum + (p.valor || 0), 0)
                          )}
                        </p>
                      )}
                    </div>
